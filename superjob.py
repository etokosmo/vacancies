...
def get_language_superjob(lang: str, headers_superjob: dict,
                          payload_superjob: dict) -> dict:
    ...
    response = requests.get(
        ...
        params=payload_superjob
    )
    response.raise_for_status()
    processed_response = response.json()

    vacancies = get_all_vacancies_superjob(
        processed_response,
        url_superjob,
        headers_superjob,
        payload_superjob
    ...
    try:
        ...
        svg_lang_salary = 0
    language = {
        "vacancies_found": processed_response.get('total'),
...
def create_language_info_superjob(langs: list, api_token: str,
                                  city: str) -> dict:
    ...
    result = {}
    for lang in langs:
        result[lang] = get_language_superjob(
            lang,
            headers_superjob,
            payload_superjob
        )
